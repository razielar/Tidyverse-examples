...
dc_14_no_ranking <- dc_14 %>% filter(is.na(ranking)) %>% group_by(affiliation) %>%
    mutate(number_fellows=n()) %>% distinct(affiliation, .keep_all = T) %>%
    select(-c(author, ranking)) %>% ungroup %>% 
    mutate(affiliation=gsub("no publications after PhD",
                            "No publications after PhD", affiliation)) %>%
    arrange(desc(number_fellows), affiliation)

## write_feather(dc_14_no_ranking, path= "feather_files/dc_14_no_ranking.feather")
...
## write_feather(dc_14_caixa_baseline_ranking,
##               path= "feather_files//dc_14_ranking.feather")


############### APPENDIX: 


## write_feather(dc_14_ranking, path = "feather_files/dc_14_ranking.feather")

## write_csv(dc_14_no_ranking, "Data/dc_14_no_ranking.csv")

## dc_14_no_ranking <- read_csv("Data//dc_14_no_ranking.csv")


dc_14_ranking <- read_feather("feather_files//dc_14_ranking.feather")


dc_14_ranking %>%
    mutate(caixa_number_fellows=
               round(caixa_number_fellows/sum(caixa_number_fellows), 4)) %>%
    mutate(MC_baseline_number_fellows=
               round(MC_baseline_number_fellows/sum(MC_baseline_number_fellows),
                     4)) 
