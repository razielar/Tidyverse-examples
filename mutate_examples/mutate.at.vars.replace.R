...
tmp %<>% select(Gene_Name, partnerRNA_gene, variable, value) %>%
    add_row(., Gene_Name="lncRNA:let7C", partnerRNA_gene="CG10283",
            variable="L3", value=2.01, .after = 4) %>%
    add_row(.,Gene_Name="lncRNA:let7C", partnerRNA_gene="CG10283",
            variable="L3", value=2.31, .after = 12)
...
tmp$Gene_Name <- factor(tmp$Gene_Name,
       levels = levels(tmp$Gene_Name)[c(2,1)])

### Not reproducible

## pdf(file = "Plots//association.lncRNA.mRNA.tmp.pdf", paper = "a4r",
##     width = 0, height = 0)

ggplot(data = tmp, aes(x=variable, y=value, group=Treatment))+
